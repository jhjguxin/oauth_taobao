...
module OauthTaobao
  module Interface
    ...
    class Users < Base
      ...
      def get_user(opts={})
        default_fields = %w(user_id uid nick sex buyer_credit
                 seller_credit location created last_visit birthday
                 type status alipay_no alipay_account alipay_account
                 email consumer_protection alipay_bind).join(',')
        opts.reverse_merge!(fields: default_fields)
        #get 'account/profile/basic.json', :params => opts
        get 'router/rest', :params => opts.merge(@client.request_params).merge(method: 'taobao.user.get')
      end
      ...
      def update(nick, sex, year, month, day, provincecode, countrycode, citycode, introduction, opts={})
        ...
        post 'account/profile/basic_update.json', :body => body,:params => @client.request_params.merge(opts)
      end
